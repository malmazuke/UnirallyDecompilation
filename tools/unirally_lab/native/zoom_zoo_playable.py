...
def original(directory):
    ...
    rows=[];finish=[None,None];loading=None;archive=None;charge_archive=None;announcements_archive=None;roll_archive=None;previous=None
    ...
    with (directory/'memory.wram').open('rb') as ws,(directory/'memory.sram').open('rb') as ss:
        for frame in range(first,last+1):
            ...
            if loading is None:
                ...
                row[7]=ord('9');archive=bytearray(row);charge_archive=w[0xd53:0xd57]
            ...
            previous=w
            row+=s[0x106f:0x1073]+s[0x618:0x61c]+charge_archive+announcements_archive+roll_archive
            rows.append(row.hex())
...
def freeze(a,b,out):
    ...
    result=dict(kind='m4_16_playable_freeze',frames=[1376,left['frames'][1]],state_bytes=680,
...
def compare(a,b,contract,binary,pack,out):
    ...
    for i in range(1,events['loading_frame']-1376):
        ...
        if any(before[a:b]!=after[a:b] for a,b in [(12,16),(581,619),(621,626),(628,632),(632,680)]):
    ...
    with tempfile.TemporaryDirectory(prefix='zoom-playable-native-') as directory:
        ...
        def execute(first,restore=None,restart=False):
            ...
            for f,line in enumerate(run.stdout.splitlines(),first):
                ...
                if int(label)!=f or len(data)!=680 or data[:8]!=b'URZZ0009' or int.from_bytes(data[8:12],'little')!=f:raise ValueError('native protocol differs')
                output.append(row)
            return output
        actual=execute(1376)
        if actual!=rows:
    ...
    result=dict(status='passed',source_commit=head.decode().strip(),source_diff_sha256=diff,binary_sha256=binary_sha,
                pack_sha256=pack_sha,contract_sha256=sha(contract.read_bytes()),frames=[1376,last],state_bytes=680,
                rows_sha256=digest(rows),restore_frames=boundaries,events=events,
                comparison_domains=dict(race=[1376,events['loading_frame']-1],race_projected_bytes=670,
                    result_archived_race_bytes=670,result_original_publication_bytes=8,semantic_loading_clock_bytes=2,
