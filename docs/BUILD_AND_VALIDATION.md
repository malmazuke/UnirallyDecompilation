...
| `content pack --rom <path> --out <ignored-pack>` | proposed (M3-02A) | Verify an exact supported ROM, reproducibly extract the scoped Classic entries and atomically create a versioned local pack with logical IDs, entry hashes and source/extractor/rules identities; never emit a plausible partial pack |
| `content pack-inspect --pack <path>` | proposed (M3-02A) | Validate pack structure, version and every required entry without a ROM; reject corruption, missing/duplicate entries and incompatible identities with a machine-readable report |
...
| `native finish-check --manifest <full-race case> [--save-frame N ...]` | implemented (M3-01 candidate) | Run the full-race native producer twice, require the canonical `URMV0001` to `URMV0002` transition and exact frozen gameplay/finish/result state through the first stable result frame, and optionally require restored suffixes to equal uninterrupted execution |
