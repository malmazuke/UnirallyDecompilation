...
M3-01 runner. M3-02 extends the same schema/profile with eleven immutable
...
base VRAM copied before the later result tile transfers, and the stable-result
palette that supersedes the earlier transition palette.
...
extension changes the exact rules identity and native inventory to twenty-four
