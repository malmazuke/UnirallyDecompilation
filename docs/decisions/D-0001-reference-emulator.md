...
Status: proposed by task M0-03 on 10 September 2026, revised 11 September 2026 after the first independent review; becomes accepted when M0-03 is accepted.
...
| Save/restore | With `Strict` synchronization, restore in a fresh process continues with identical WRAM/registers at all 42 swept save points; saving itself alters the continuing run at 7 points inside frames 45–90 of the boot script, which the tool detects; with bsnes' default `Fast` method restores silently diverged at 2 of 9 points. Video of the first restored frame and the resampled audio stream are not part of the state | Restore in a fresh process continued identically at the one point tried (65680-byte state) |
