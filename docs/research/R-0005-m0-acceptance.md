...
| 10 | Synthetic checks | Re-run: 118/118 on this host at `abb8c60`, `source.dirty` false. On Linux the last observed result is 117/117 at `5498608` (CI run 34557563391). **CI has not yet run on `abb8c60`** — the coordinator pushes the branch | **Outstanding until CI is green on the candidate** |
...
| **Total** | **~9 h 55 min** | **11** | **≥1 h** (5 of 11 sessions recorded no duration) | **6 of 11** |
...
Every M0 gate has at least one observation behind it, and the observations reproduce today on the candidate commit: the re-run produced the recorded digests and exit codes with no adjustment. The single outstanding item is CI on this exact commit, which is the coordinator's to run; until it is green on `ubuntu-24.04` and `macos-15`, M0 acceptance is blocked. That is the honest state — not "M0 is accepted pending CI".
